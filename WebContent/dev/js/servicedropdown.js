		...
		if (product_capacity=="0" || product_capacity=="default" || product_capacity==""){
			...
			$("#product_brand").html("<option>Loading ... </option>");
			 ...
			 $.ajax({					 
				type: "GET",
				url: "/bookbattery/servlet/Functions?hidWhatToDo=Fx_Get_Product_Brand",
				//data: {vehiclemodel:product_capacity, keyword:brandkeyword,flag:"inverterbattery" },
				success: function(data)
				{	
					if(data.split("value").length<=4)
					{
...
{	
	if (!$("#product_type_page").val()=="0" || !$("#product_type_page").val()=="" || !$("#product_type_page").val()=="null" || !$("#product_type_page").val()=="undefined"|| !$("#product_type_page").val()==undefined )
	{
					...
					if (!$("#product_capacity_page").val()=="0" || !$("#product_capacity_page").val()=="" || !$("#product_capacity_page").val()=="null" || !$("#product_capacity_page").val()=="undefined"|| !$("#product_capacity_page").val()==undefined) 
					{
						...
						var brandkeyword=$("#brandkeyword").val();
						 ...
						 ({					 
							type: "GET",
							url: "/bookbattery/servlet/Functions?hidWhatToDo=Fx_Get_Product_Brand",
							//data: {vehiclemodel:$("#product_capacity").val(), keyword:brandkeyword,flag:"inverterbattery" },
							success: function(data)
							{
		...
		else if ($("#product_type").val()=="Inverter")
		{
			$.ajax
			({
				type: "GET",
		...
		else
		{
			...
			({					 
				...
				{	
					...
					{
						$("#product_make").html("<option value='All'>ALL</option>"+data);
						product_make=$("#product_make").val();
					}
					else
					{						
						...
						({					 
							type: "GET",
							url: "/bookbattery/servlet/BatteryHome?hidWhatToDo=getvehicle_model",
							data: {vehiclename: $("#product_make").val(), batterytype:$("#product_type").val()},
							success: function(data)
							{	
								...
								if (!$("#product_model_page").val()=="0" || !$("#product_model_page").val()=="" || !$("#product_model_page").val()=="null" || !$("#product_model_page").val()=="undefined"|| !$("#product_model_page").val()==undefined) 
								{
									...
									if($("#product_model_page").val()=="All" || $("#product_model_page").val()=="All")
									{
									//alert("inside if");
									$("#product_model").html("<option value='All'>ALL</option>"+data);
									product_model=$("#product_model").val();
									}
									else
									{
									...
									({					 
										type: "GET",
										url: "/bookbattery/servlet/Functions?hidWhatToDo=Fx_Get_Product_Brand",
										//data: {vehiclemodel:$("#product_model").val(),keyword:brandkeyword },
										success: function(data)
											{	
												$("#product_brand").html(data);
...
var input_type;
	...
	// ############# Important Code ########################//
	// ############# Code Disabled by Sai Krishna to Stop Visitor Mobile Number Capture 22-Oct-2016 ########################//
	// var MobileNumberPopUpCheck =$("#MobileNumberPopUpCheck").val();
	// if (MobileNumberPopUpCheck=="HomePage")
	// {
		//## Getting Mobile number for Cookie
		// var user_mobile_number_cookie_tmp=getCookie('user_mobile_number_cookie');
		
		//## Sending the value to temp ID
		// $("#user_mobile_number_cookie_tmp").val(user_mobile_number_cookie_tmp);
		
		//## Validation for Cookie mobile number.  
		// if (mobilenumberValidation("","user_mobile_number_cookie_tmp",input_type) == false)
		// {
			// user_mobile_number_cookie_tmp="";
		// }
		// else{
			// UserMobileNumber_Cookie_Tmp=user_mobile_number_cookie_tmp;
		// }
		
		// if (user_mobile_number_cookie_tmp=="" || user_mobile_number_cookie_tmp=="null" || user_mobile_number_cookie_tmp=="undefined" || user_mobile_number_cookie_tmp==undefined || user_mobile_number_cookie_tmp==null)
		// {
			// AskforMobileNumber_Pop_Up()
			// return;
		// }
		// else
		// {
			// RequestProductDetails("Cookie_Number")
			// return;
		// }
	// }
	// else{
...
function RequestProductDetails(RequestType){
	...
	// if (RequestType=="Cookie_Number" || RequestType=="Cookie"){
		// UserMobileNumber=UserMobileNumber_Cookie_Tmp;
	// }
	// else{
		// UserMobileNumber =$("#home-page-user-mobile-number").val();
		
		// var input_type="mobilenumber";
		// if (mobilenumberValidation("","home-page-user-mobile-number",input_type) == false)
		// {
			// return;
		// }
		// else
		// {
			// setCookie("user_mobile_number_cookie", UserMobileNumber);
		// }
	// }
	// ############# Important Code ########################//
	// ############# Code Disabled by Sai Krishna to Stop Visitor Mobile Number Capture 22-Oct-2016 ########################//
...
function RequestProductDetails_Step_2()
{
	...
	if (product_type_URL=="Inverter-Batteries" || product_type=="Inverter Batteries"){
		...
		window.location.href = publicUrl+'/bookbattery/'+product_type_URL+'/'+product_make_URL+'/'+product_model_URL+'/'+product_state_URL+'/'+product_city_URL+'/'+product_brand_URL+'/'+Filter_Battery_Warranty_URL+'/' ;
	}
}
