					...
					<table class="table table-bordered table-striped table-hover dataTable js-exportable">
						...
						<tbody>
								...
								%>	
									...
									<% } %>
									<td><button type="button" class="btn btn-primary btn-block waves-effect" data-trigger="focus" data-container="body" data-toggle="popover" data-placement="right" title="Order Details"  data-html="true" data-content="<strong>Payment Mode:</strong><%=payment_mode%><br /><strong>Address:</strong><%=customer_address%>"><%=ordnum%></button></td>
										...
										<td><%=payment_mode%> </td>
										<td><select style='width:110px; height:30px;color:#000;' name="<%=ord_id%>" id="<%=ord_id%>" onChange="javascript:funtoUpdate('<%=ord_id%>','<%=ordnum%>','<%=state%>','<%=city%>','<%=bat_type%>','<%=customer_mobnumber%>','<%=payment_mode%>','<%=order_type%>')"><option value=''>Update Status</option><option value='Customer Contacted'>Customer Contacted</option><option value='Customer Not Contacted'>Customer Not Contacted</option><option value='Postponed'>Postpone</option><option value='Repeated'>Repeated</option></select></td>
