...
Vector alist=(Vector)session.getAttribute("sesMisTrolleyOrdersVector");
			...
			<div class="body">
				<div class="table-responsive">
					<table class="table table-bordered table-striped table-hover dataTable js-exportable">
						<thead class="header bg-blue-grey">
							<tr>
								<th>Order No</th>
								<th>Customer Name<br>Number<br/>Email</th>
								<th>Trolley Brand/Model</th> 
 								<th>Price</th>
								<th>Location</th>
 								<th>Order Status</th>
 								<th>Order Reasons</th>
						...
						%>	
							<%
								for(int i=0;i<alist.size();i++)
								{
									...
									String trolley_brand = String.valueOf(ht.get("trolley_brand"));
									String trolley_model = String.valueOf(ht.get("trolley_model"));
							...
							%>
								<% if(order_reasons.equals("installed")) {%>
									...
									<td>
										 <%=trolley_brand%> - <%=trolley_model%>
									</td>
									 
									<td><strong>Price:</strong> <%=price%><br/><strong>QTY:</strong> <%=quantity%></td>
									...
									<td>
                                     <select style='width:110px; height:30px;color:#000;' name="<%=ordid%>" id="<%=ordid%>" onChange="javascript:funtoUpdate('<%=ordid%>','<%=ordnum%>','<%=state%>','<%=city%>','<%=area%>','<%=customer_mobnumber%>','<%=payment_mode%>')"><option value=''>Update Status</option><option value='Customer Contacted'>Customer Contacted</option><option value='Customer Not Contacted'>Customer Not Contacted</option><option value='Postponed'>Postpone</option><option value='Repeated'>Repeated</option></select>
									...
									<td><%if(order_status.equals("installed"))
											...
											{
											%>
											<button class="btn bg-pink waves-effect" type="button" onclick="generateInvoice('<%=ordnum%>')"><i class="material-icons">create</i> <span class="icon-name">Generate</span></button>
											<button class="btn bg-blue waves-effect" type="button" style="margin-top:5px;" onclick="viewInvoice('<%=ordnum%>')"><i class="material-icons">file_download</i> <span class="icon-name">View</span></button>		
											<button class="btn bg-teal waves-effect" type="button" style="margin-top:5px;" onclick="CheckInvoice('<%=ordnum%>')"> <i class="material-icons">send</i><span class="icon-name">Send</span> </button>				
											<%
											}
											%>
										</td>
