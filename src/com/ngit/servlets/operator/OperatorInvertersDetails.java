...
public class OperatorInvertersDetails extends HttpServlet 
{  
	...
	* This action is used to  insert consumer details.
	...
	if(strWhatToDo.equalsIgnoreCase("insertconsumerorderdetails"))
		{ 
			try
			{
				String strRes=insertconsumerorderdetails(req,res,session,domainname,strIpAddress,strPort,SMSFromAddress,FromEmailId,strsuppemaild,baseUrl,strsuppmobnumber,strCMSSERVERIP,strPDFFilePath,strPDFRelFilePath,struserName,OperatorTeamCount);
				LogLevel.DEBUG(5, new Throwable(), "strRes :" + strRes);
				out.println(strRes);
				out.close();
	...
	public String getinverterdetails(HttpServletRequest req,HttpServletResponse res,HttpSession session,String strIpAddress,String strPort,String strsuppmobnumber,String strsuppmobnumber1,String SMSFromAddress,String OperatorTeamCount)
	{
		...
		{	
			...
			{ 
				...
				res.sendRedirect("/bookbattery/jsp/operator/inverter/orderinverter.jsp");
				return strRes;
			}
			else
			{
				...
				res.sendRedirect("/bookbattery/jsp/operator/inverter/inverterdetails.jsp?invertertype="+strinvertertype+"&inverterbrand="+inverterbrand+"&invertercapacity="+invertercapacity+"&city="+strcity+"&pincity="+city+"&strarea="+area+"&strstate="+strstate+"&strpincode="+strpincode+"&sortpricess="+sortpricess);
				return strRes;
			}
		}
		catch (Exception e)
...
{ 	
	...
	{		
		...
		else
		{
			...
			{
				strRes="<table border='0' width='300px' height='10px' cellpadding='0' cellspacing='0' bgcolor='#88689f'><tr class='top1'><td>&nbsp;<font color='#FFFFFF'>BookBattery</td><td align='right'><a href='javascript:closemobdiv(greyout(false));'><img src=\"/bookbattery/images/Delete1.png \" border='0'></a></td></tr></table><table border='0' width='300px' height='2px' bgcolor='#FFFFFF' valign'top'><tr class='pages' bgcolor='#FFFFFF'><td align='center' bgcolor='#FFFFFF'>No Retailers Found on Selected City.</br></td></tr><tr height='20'><td></td></tr><tr height='20'><td align='center'><input type='button' name='emailok' id='emailok' value='OK' onclick='closemobdiv();' class='button4'><br></td></tr><tr height='15'></tr></table>";
			}
			else
			{
			...
			strRes="<table border='0' width='300px' height='10px' cellpadding='0' cellspacing='0' bgcolor='#88689f'><tr class='top1'><td>&nbsp;<font color='#FFFFFF'>BookBattery</td><td align='right'><a href=\"javascript:navigatetobatterywale('"+emailid+"','"+strUsermobileno+"','"+pw+"','"+strusername+"','"+usercity+"','"+strstate3+"','inverter','','','"+inverterbrand+"','"+invertermodel+"','"+discountprice+"','','1','"+strretname+"','"+strretmobnum+"');\"><img src=\"/bookbattery/images/Delete1.png \" border='0'></a></td></tr></table><table border='0' width='300px' height='2px' bgcolor='#FFFFFF' valign'top'><tr bgcolor='#FFFFFF'><td align='justify' bgcolor='#FFFFFF' style='font-family:Verdana;font-size:11px;color:#000000;'>Your BookBattery Inverter Ord Ref No: <b>"+verificationcode+"</b>. <br>Inverter Model: <b>"+invertermodel+"</b>. <br>MRP: <b>"+price+"</b> Discount Price: <b>"+discountprice+"</b>. <br>"+strretname+", Mob No-"+strretmobnum+" will fullfill your order.</td></tr><tr height='20'><td></td></tr><tr height='20'><td align='center'><input type='button' name='emailok' id='emailok' value='OK' class='button4' onclick=\"javascript:closemobdiv(greyout(false));\"><br></td></tr><tr height='15'></tr></table>";
				...
				else
				{
					...
					String strSqlQry = "insert into inverter_order_details(order_id,order_number,consumer_name,consumer_mobnumber,consumer_emailid,consumer_verify_code,retailer_name,retailer_mobile_number,retailer_emailid,state,city,inverter_brand,inverter_model,inverter_capacity,price,area,pincode,operator,creation_date,agent_name,referred_coupon_code,confirm_by)values(NULL,'"+verificationcode+"','"+strusername+"','"+strUsermobileno+"','"+emailid+"','0','"+strretname+"','"+strretmobnum+"','"+strretemail+"','"+strstate3+"','"+pincity+"','"+inverterbrand+"','"+invertermodel+"','"+invertercapacity+"','"+discountprice+"','"+strarea+"','"+strpincode+"','"+orderedby+"',now(),'"+struserName+"','"+refcode+"','Operator')";
					LogLevel.DEBUG(5,new Throwable()," : "+strSqlQry);
					int reslt = qm.executeUpdate(strSqlQry);
				}
			...
			if(Get_Product_Price_SQL_HT.isEmpty())
			{
				...
				String strSqlQry_Update = "UPDATE inverter_order_details SET erp_pre_tax = '"+ERP_Pre_TAX+"', MRP_Price = '"+ProductActualPrice+"', city_percentage = '"+City_Percentage+"', consumer_address='"+User_Address_Landmark+"', quantity='"+Quantity+"' , payment_mode='"+paymenttype+"', payment_mode_type='"+paymenttype+"'  WHERE  order_number='"+verificationcode+"' ";
