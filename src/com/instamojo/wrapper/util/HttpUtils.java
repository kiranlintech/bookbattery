...
public class HttpUtils {
    ...
    public static String sendPostRequest(String url, Map<String, String> headers, Map<String, String> params) throws IOException {
		...
		LogLevel.DEBUG(5, new Throwable(), "url :" + url);

        HttpPost httpPost = new HttpPost(url);

        addHeadersToHttpRequest(httpPost, headers);
		LogLevel.DEBUG(5, new Throwable(), "params :" + params);
		
        if (params != null && params.size() > 0) {
            List<NameValuePair> nameValuePairs = new ArrayList<>();
            for (Map.Entry<String, String> param : params.entrySet()) {
				LogLevel.DEBUG(5, new Throwable(), "Getting IN");
				LogLevel.DEBUG(5, new Throwable(), "Getting IN :"+ param);
                nameValuePairs.add(new BasicNameValuePair(param.getKey(), param.getValue()));
            }
			LogLevel.DEBUG(5, new Throwable(), "nameValuePairs :" + nameValuePairs);
            httpPost.setEntity(new UrlEncodedFormEntity(nameValuePairs));
        }

        HttpClient httpClient = HttpClientBuilder.create().build();
		LogLevel.DEBUG(5, new Throwable(), "httpClient :" + httpClient);

        HttpResponse httpResponse = httpClient.execute(httpPost);
		LogLevel.DEBUG(5, new Throwable(), "httpResponse :" + httpResponse);
        return getHttpResponseAsString(httpResponse);
    }
