                    ...
                    <div class="card">
                        ...
                        <div class="body">
                               <div class="row clearfix">
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                         <button type="button" class="btn bg-blue-grey waves-effect" onclick="GetTrolleyDetails()">
											<i class="material-icons">search</i>
											<span>GET TROLLEY DETAILS</span>
										</button>  
                                    </div>
                                </div>
                            </div>
                            <div class="row clearfix" id="mistrolley_details"></div>
						</div>
                    </div>
                </div>
            </div>
...
function GetTrolleyDetails(){
	...
	$.ajax({
			type: "GET",
			url: "/bookbattery/servlet/BatterystoreMISReports?hidWhatToDo=trolleyorderdetails",
			data: {todate: todate, fromdate: fromdate },
			success: function(data){
				//alert(data);
				$("#mistrolley_details").html(data);
			}
	}); 
}
...
function funtoUpdate(orderid,ordernumber,state,city,area,cusmobile,PaymentMode){
	...
	else
	{
		var xmlhttp= "";
		var resp= "";
		var url ="/bookbattery/servlet/BatterystoreTrolleyDetails?hidWhatToDo=updateorderstatus&orderid="+orderid+"&ordernumber="+ordernumber+"&orderstatus="+orderedstatus;
 ...
 function CustomerContacted(orderid,ordernumber,orderstatus,state,city,cusmobile,PaymentMode){
	...
	else
	{
		// DO Noting
			...
			var OrderLable ="Update Trolley Order Status";
	...
	//alert(ordnum);
		$.ajax({
			...
			success: function(data){
				...
				var AfterOrder_PopUp="<a class='btn hide' data-toggle='modal' data-target='.after_order_popup' data-toggle='modal' data-backdrop='static' data-keyboard='false' id='after_order_popup_btn'></a><div class='modal fade after_order_popup' tabindex='-1' role='dialog' aria-labelledby='after_order_popup_label'> <div class='modal-dialog' role='document'> 	<div class='modal-content'> <div class='modal-header bg-red'><h4 class='modal-title' id='after_order_popup_label'>"+OrderLable+"</h4><button type='button' onclick='okey_done()' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'> x</span></button> 	</div> <div class='modal-body'><div class='form-group'> <div id='after_order_popup_data'> "+OrderMessage+"</div> <div style='text-align: center;'><button type='button' class='btn bg-red waves-effect' data-dismiss='modal' onclick='GetTrolleyDetails()'>Okay</button> </div></div> </div></div> </div> </div>";
		...
		$.ajax({
			...
			success: function(data){
				...
				if(data.indexOf("No Details found")>=0){
					$("#defaultModal").html("");
					var AfterOrder_PopUp="<div class='modal-dialog' role='document'><div class='modal-content'> <div class='modal-header bg-green'><h4 class='modal-title' id='after_order_popup_label'>Confirmation Message - BookBattery</h4><button type='button' data-dismiss='modal' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'> x</span></button></div> <div class='modal-body'> 	<div class='form-group'> <div id='after_order_popup_data'>Invoice Not Generated for the Order. Please Generate Invoice.<br/></br/></div> <div style='text-align: center;'><button type='button' class='btn bg-green waves-effect' data-dismiss='modal'>Okay</button> </div></div> </div></div> </div>";
