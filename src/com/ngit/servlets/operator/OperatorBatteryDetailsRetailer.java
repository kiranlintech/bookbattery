...
public class OperatorBatteryDetailsRetailer extends HttpServlet 
{  
	...
	* @strSqlQry1 : Query has multiple pages to select battery details from battery_details table.
	...
	public String getbatdetails(HttpServletRequest req,HttpServletResponse res,HttpSession session)
	{
		...
		{	
			...
			{
				strConditions1= " and bat_brand='"+strbatbrand+"' ";
			}
			else
			{	
				strConditions1= "";
			...
			if(!strbatbrand.equals("All") )
			{
				strConditionsDetails= " and a.bat_brand='"+strbatbrand+"' ";
			}
			else
			{	
			...
			//String strSqlQrym ="select bat_act_price,bat_witbat_price from batteryprice where bat_brand='"+strbatbrand+"' and bat_model='"+bat_model+"' and state='"+strstate+"'";
			...
			{ 
				...
				res.sendRedirect("../jsp/operator/orderbattery/orderbatteryretailer.jsp");
				return strRes;
			}
			else
			{
				...
				res.sendRedirect("../jsp/operator/orderbattery/orderbatterydetailsretailer.jsp?batterytype="+strbattype+"&vehiclemake="+strvehmake+"&vehiclemodel="+strvehmodel1+"&batterybrand="+strbatbrand+"&city="+strcity+"&pincity="+city+"&strarea="+strarea+"&strstate="+strstate+"&strpincode="+strpincode+"&visid="+visid+"&retailer="+retailer);
				return strRes;
			}
			//}
		}
		...
		else
		{
          ...
          // Put parts in message  
			...
			LogLevel.DEBUG(5, new Throwable(), "strPdfURL : " + strPdfURL);
					...
					if(htgetoperatorname1.isEmpty())
					{
							...
							if(htgetoperatorname12.isEmpty())
							{
								...
								if(htgetoperatorname123.isEmpty())
								{
									...
									String strSqlQry = "insert into battery_order_details(ord_id,order_number,consumer_name,consumer_mobnumber,consumer_emailid,retailer_name,retailer_mobilnumber,retailer_emailis,state,city,battery_brand,battery_model,price,area,pincode,pdfurl,bat_type,veh_name,veh_model,witholdbatprice,operator,creation_date,agent_name,payment_mode,order_agent_comments,payment_mode_type,forwareded_order,delivery_mode,delivery_charge,order_coupon_code,referred_coupon_code,coupon_code_expiry_date)values(NULL,'"+verificationcode+"','"+strusername+"','"+strUsermobileno+"','"+emailid+"','"+strretname+"','"+strretmobnum+"','"+strretemail+"','"+strstate3+"','"+pincity+"','"+batterybrand+"','"+batterymodel+"','"+price+"','"+strarea+"','"+strpincode+"','"+strPdfURL+"','"+batterytype+"','"+vehiclemake+"','"+vehiclemodel+"','"+withbatprice+"','"+orderedby+"',now(),'"+agent_name+"','"+payment_mode+"','"+order_agent_comments+"','"+payment_mode_type+"','"+forwareded_order+"','"+deliverytype+"','"+delivery_charge+"','"+order_coupon_code+"','"+referred_coupon_code+"','"+coupon_code_expiry_date+"')";
				...
				{
					String strSqlQry = "insert into battery_order_details(ord_id,order_number,consumer_name,consumer_mobnumber,consumer_emailid,retailer_name,retailer_mobilnumber,retailer_emailis,state,city,battery_brand,battery_model,price,area,pincode,pdfurl,bat_type,veh_name,veh_model,witholdbatprice,operator,creation_date,agent_name,payment_mode,order_agent_comments,payment_mode_type,forwareded_order,delivery_mode,delivery_charge,order_coupon_code,referred_coupon_code,coupon_code_expiry_date)values(NULL,'"+verificationcode+"','"+strusername+"','"+strUsermobileno+"','"+emailid+"','"+strretname+"','"+strretmobnum+"','"+strretemail+"','"+strstate3+"','"+pincity+"','"+batterybrand+"','"+batterymodel+"','"+price+"','"+strarea+"','"+strpincode+"','"+strPdfURL+"','"+batterytype+"','"+vehiclemake+"','"+vehiclemodel+"','"+withbatprice+"','"+orderedby+"',now(),'"+struserName+"','"+payment_mode+"','"+order_agent_comments+"','"+payment_mode_type+"','"+forwareded_order+"','"+deliverytype+"','"+delivery_charge+"','"+order_coupon_code+"','"+referred_coupon_code+"','"+coupon_code_expiry_date+"')";
					LogLevel.DEBUG(5,new Throwable()," : "+strSqlQry);
					int reslt = qm.executeUpdate(strSqlQry);
				}
			...
			else
			{
				...
				String strSqlQry_Update = "UPDATE battery_order_details SET erp_pre_tax = '"+ERP_Pre_TAX+"', MRP_Price = '"+ProductActualPrice+"', city_percentage = '"+City_Percentage+"', consumer_address='"+User_Address_Landmark+"', battery_capacity='"+Battery_Capacity+"', payment_mode='"+payment_mode+"', payment_mode_type='"+payment_mode+"', quantity='"+Quantity+"', order_type='"+Order_battery_type+"'  WHERE  order_number='"+verificationcode+"' ";
			...
			{
				//######## Send SMS for Forwrded ORDER
				Order_SMS Send_Order_SMS_Checkout = new Order_SMS(qm);
				String SMS_Report=Send_Order_SMS_Checkout.Send_Order_SMS(req,res,session,verificationcode,"Battery","No","No","No");
	...
	* @strSqlQry : carries the query to vehicle model details in vehicle_details table.
	...
	String strRes="";
		...
		{	
			...
			strSqlqry = "select distinct retailer_id,retailer_name from retailer_location_mapping where state='"+state+"' and city='"+city+"' order by retailer_name asc;"; 
			LogLevel.DEBUG(5, new Throwable(), "strSqlqry :" + strSqlqry);
