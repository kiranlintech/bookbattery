...
var Service_Price_Discount_RXT=$("#service_discount_price_page").val();
var Service_Price_MRP_RXT=$("#service_price_page").val();
var order_services_type = $("#product_services_type_page").val();
var service_place_select = $("#service_place_select").val();
var order_vehiclemake = $("#vehicle_make_page").val();
var order_vehiclemodel = $("#vehicle_model_page").val();
var product_type = $("#product_type").val();
battery_type = $("#product_type").val();
//alert("battery_type"+battery_type);
var product_capacity = $("#product_capacity").val();
var order_price = $("#service_price_page").val();
var order_discount_price = $("#service_discount_price_page").val();
...
var FinalService_Price_MRP_RXT="";
		  ...
		  <div class='modal-body'>\
			...
			<form role='form'>\
							...
							<h4 class='hide'> Personal Details</h4>\
							  ...
							  <div id='order-form-usermobile-otp-div' style='display:none;'>\
								  <div class='form-group'>\
									...
									<input type='tel' class='form-control' id='order-form-usermobile-otp' placeholder='XXXXXX' maxlength='6'  onkeydown=\"if ((event.which && event.which == 13) || (event.keyCode && event.keyCode == 13)){javascript:VerifySmsConfirmation(); return false; } else return true;\">\
						...
						<div class='col-md-12'>\
							<h4 class='hide'> Location</h4>\
								...
								<div class='form-group' id='order-form-area-div'>\
									...
									<select class='form-control yes order-form-area' id='order-form-area' data-live-search='true' data-show-subtext='true' onChange='find_retaileres_avaliable();' onClick='scroll_on_area();'>\
									  <option>Please Select Area</option>\
									</select>\
									<div id='order-form-area-error'style='display:none;'></div>\
									</br><small id='order-form-area-error-help' class='form-text text-muted'>The Area Selection does not have to be Exact Match of Your Delivery Address. Select the Area in the List which is closest to Your Delivery Address.</small>\
								</div>\
				...
				<div class='row setup-content' id='step-3'>\
					<div class='col-xs-12'>\
						<div class='col-md-12'>\
							<h4 class='' style='padding-left: 40px;'> Select Payment Mode</h4>\
							<hr style='margin: 5px;'>\
							<div class='form-group'>\
								<div class='' style='padding-left: 40px;'>\
									<div class='payment_radio'>\
										<div class='payment_radio-success'>\
											<input type='radio' name='Payment_Mode' id='Payment_Mode-1'  value='Cash On Delivery' />\
											<label for='Payment_Mode-1'><span class='icon-rupee payment-mode-icon'> </span>&nbsp;Cash On Delivery</label>\
										</div>\
										<div class='payment_radio-success'>\
											<input type='radio' name='Payment_Mode' id='Payment_Mode-7'  value='Online Payment After Fitment' />\
											<label for='Payment_Mode-7'><span class='icon-rupee payment-mode-icon'> </span>&nbsp; Online Payment After Fitment</label>\
										</div>\
										<div class='payment_radio-success'>\
											<input type='radio' name='Payment_Mode' id='Payment_Mode-2' value='Debit Card'/>\
											<label for='Payment_Mode-2'><span class='icon-credit-card payment-mode-icon'> </span>&nbsp;Debit Card</label>\
										</div>\
										<div class='payment_radio-success'>\
											<input type='radio' name='Payment_Mode' id='Payment_Mode-3'  value='Credit Card'/>\
											<label for='Payment_Mode-3'><span class='icon-credit-card payment-mode-icon'> </span>&nbsp;Credit Card</label>\
										</div>\
										<div class='payment_radio-success'>\
											<input type='radio' name='Payment_Mode' id='Payment_Mode-4' value='Net Banking' />\
											<label for='Payment_Mode-4'><span class='icon-laptop payment-mode-icon'> </span>&nbsp;Net Banking</label>\
										</div>\
										<div class='payment_radio-success'>\
											<input type='radio' name='Payment_Mode' id='Payment_Mode-5'  value='UPI - Unified Payment Interface' />\
											<label for='Payment_Mode-5'><span class='icon-money payment-mode-icon'> </span>&nbsp;UPI - Unified Payment Interface</label>\
										</div>\
										<div class='payment_radio-success hide '>\
											<input type='radio' name='Payment_Mode' id='Payment_Mode-6'  value='Digital Wallets' />\
											<label for='Payment_Mode-6'><span class='icon-money payment-mode-icon'> </span>&nbsp;Digital Wallets (Ola Money, Mobikwik, Freecharge)</label>\
										</div>\
									</div>\
...
$(document).ready(function() {
	...
	FinalService_Price_MRP_RXT=order_price;
...
{	
    var service_place_select=$('#service_place_select').val();
        ...
        //alert("1"+order_discount_price);

    $('#final_price').html("<i class='icon-inr' aria-hidden='true'></i>&nbsp;"+FinalService_Price_Discount_RXT);
	$('#discount_price').html("<i class='icon-inr' aria-hidden='true'></i>&nbsp;"+FinalService_Price_Discount_RXT);
}	
}
...
function SendSmsConfirmation()
{
	...
	({					 
		type: "GET",
		url: "/bookbattery/servlet/Functions_Service?hidWhatToDo=GetVerificationSms",
		data: {UserMobileNumber:order_form_usermobile_number},
		...
		{	
			...
			if (!data == "")
			{
				...
				else
				{
					...
					if(SendSmsConfirmation_limit==0)
					{
						...
						$("#order-form-usermobile-otp-help").html(" SMS sent to +91"+order_usermobile_number+", This OTP will be valid for 15 Minutes.");
						return;
					}
					else if(SendSmsConfirmation_limit==1)
					{
						SendSmsConfirmation_limit=SendSmsConfirmation_limit+1;
						$("#order-form-usermobile-otp-help").html("<p style='color: red;'> SMS sent 2nd time to +91"+order_usermobile_number+", This OTP will be valid for 15 Minutes.</p>");
            ...
            if(product_type=="Car Batteries" || product_type=="Bike Batteries")
			{
				//current_order_details_after_order_popup="<tr> <th>Service Type&nbsp;:</th> <td>"+order_services_type+"</td> </tr> <tr> <th>Service Packages&nbsp;:</th> <td>"+order_services_package+"</td> </tr> <tr> <th>Price Details&nbsp;:</th> <td>Price - Rs. "+order_price+"</br>Discount Price - Rs. "+order_discount_price+"</td> </tr><tr> <th>Vehicle Details&nbsp;:</th> <td> "+order_vehiclemake+", "+order_vehiclemodel+"</td> </tr> ";
				current_order_details_after_order_popup="<tr> <th>Service Type&nbsp;:</th> <td>"+order_services_type+"</td> </tr> <tr> <th>Battery Type&nbsp;:</th> <td>"+product_type+"</td> </tr><tr> <tr> <th>Service Place&nbsp;:</th> <td>"+$('#service_place_select').val().replace("_"," ")+"</td> </tr><th>Price Details&nbsp;:</th> <td>Price - Rs. "+FinalService_Price_Discount_RXT+"</td> </tr><tr> <th>Vehicle Details&nbsp;:</th> <td> "+order_vehiclemake+", "+order_vehiclemodel+"</td> </tr><tr> <th>Battery Quantity&nbsp;:</th> <td> "+order_product_qty+"</td> </tr>";
			}			
			else
			{
				//current_order_details_after_order_popup="<tr> <th>Service Type&nbsp;:</th> <td>"+order_services_type+"</td> </tr> <tr> <th>Service Packages&nbsp;:</th> <td>"+order_services_package+"</td> </tr> <tr> <th>Price Details&nbsp;:</th> <td>Price - Rs. "+order_price+"</br>Discount Price - Rs. "+order_discount_price+"</td> </tr><tr> <th>Battery Capacity&nbsp;:</th> <td> "+product_capacity+"</td> </tr> ";
				current_order_details_after_order_popup="<tr> <th>Service Type&nbsp;:</th> <td>"+order_services_type+"</td></tr><tr> <th>Battery Type&nbsp;:</th> <td>"+product_type+"</td> </tr><tr> <th>Service Place&nbsp;:</th> <td>"+$('#service_place_select').val().replace("_"," ")+"</td> </tr><tr> <th>Price Details&nbsp;:</th> <td>Price - Rs. "+FinalService_Price_Discount_RXT+"</td> </tr><tr> <th>Battery Quantity&nbsp;:</th> <td> "+order_product_qty+"</td> </tr><tr> <th>Battery Capacity&nbsp;:</th> <td> "+product_capacity+"</td> </tr> ";
			}
	...
	if(InsertBeforeOrder_limit==0)
	{
		...
		//alert(FinalService_Price_Discount_RXT);
		//alert(FinalService_Price_MRP_RXT);
		order_discount_price=$('#service_discount_price_page').val();
		service_place_select=$('#service_place_select').val();
		var order_usermobile_number_URL = order_form_usermobile_number.replace(/ /g, "+");
		var order_services_type_URL = order_services_type.replace(/ /g, "+");
		var service_place_select_URL = service_place_select.replace(/ /g, "+");
		...
		var order_price_URL = order_price.replace(/ /g, "+");
		var order_discount_price_URL = order_discount_price.replace(/ /g, "+");
		var product_type_URL = product_type.replace(/ /g, "+");
		
		//alert(order_discount_price_URL);
		if(order_product_qty=="" || order_product_qty=="undefined" || order_product_qty==undefined)
		{
			order_product_qty="1";
		}
		else
		...
		order_services_type_URL=escape(order_services_type_URL);
		service_place_select_URL=escape(service_place_select_URL);
		...
		order_price_URL=escape(order_price_URL);
		order_discount_price_URL=escape(order_discount_price_URL);
		order_product_qty_URL=escape(order_product_qty_URL);
		product_type_URL=escape(product_type_URL);
		
		//alert(product_type_URL);


		$.ajax
		({					 
			url: "/bookbattery/servlet/Functions_Service?hidWhatToDo=InsertBeforeOrder",
			type: 'GET',
			data:{	UMN:order_usermobile_number_URL,
					UOTP:order_usermobile_otp,
					USeT:order_services_type_URL,
					USP:service_place_select_URL,
					UVMK:order_vehiclemake_URL,
					UVMD:order_vehiclemodel_URL,
					UST:order_state_URL,
					UCT:order_city_URL,
					UMRP:order_price_URL,
					UDP:order_discount_price_URL,
					UQTY:order_product_qty_URL,
					UPT:product_type_URL,
			},
			success: function(data)
			{	
				console.log(data);
...
function VerifySmsConfirmation()
{
	...
	{	
		...
		{*/
			...
			{
				//current_order_details_after_order_popup="<tr> <th>Service Type&nbsp;:</th> <td>"+order_services_type+"</td> </tr> <tr> <th>Service Packages&nbsp;:</th> <td>"+order_services_package+"</td> </tr> <tr> <th>Price Details&nbsp;:</th> <td>Price - Rs. "+order_price+"</br>Discount Price - Rs. "+order_discount_price+"</td> </tr><tr> <th>Vehicle Details&nbsp;:</th> <td> "+order_vehiclemake+", "+order_vehiclemodel+"</td> </tr> ";
				current_order_details_after_order_popup="<tr> <th>Service Type&nbsp;:</th> <td>"+order_services_type+"</td> </tr> <tr> <th>Price Details&nbsp;:</th> <td>Price - Rs."+FinalService_Price_Discount_RXT+"</td> </tr><tr> <th>Vehicle Details&nbsp;:</th> <td> "+order_vehiclemake+", "+order_vehiclemodel+"</td> </tr> ";
			}			
			else
			{
				//current_order_details_after_order_popup="<tr> <th>Service Type&nbsp;:</th> <td>"+order_services_type+"</td> </tr> <tr> <th>Service Packages&nbsp;:</th> <td>"+order_services_package+"</td> </tr> <tr> <th>Price Details&nbsp;:</th> <td>Price - Rs. "+order_price+"</br>Discount Price - Rs. "+order_discount_price+"</td> </tr><tr> <th>Battery Capacity&nbsp;:</th> <td> "+product_capacity+"</td> </tr> ";
				current_order_details_after_order_popup="<tr> <th>Service Type&nbsp;:</th> <td>"+order_services_type+"</td> </tr><tr> <th>Price Details&nbsp;:</th> <td>Price - Rs."+FinalService_Price_Discount_RXT+"</td> </tr><tr> <th>Battery Quantity&nbsp;:</th> <td> "+order_product_qty+"</td> </tr><tr> <th>Battery Capacity&nbsp;:</th> <td> "+product_capacity+"</td> </tr> ";
			}
...
function Book_Service_Online_Fun()
{
	//alert("inside");
	var product_state =$("#product_state").val();
	var product_city =$("#product_city").val();	
	if (product_state=="Karnataka" && product_city=="Bangalore")
	{
		$('#loc_div').hide();
		Book_Service_Online_Fun_bangalore();
	}
	else
	{
		//$('#loc_div').removeClass('hide');
		//$('#loc_div').fadeIn('fast').delay(1000);
		//$('#loc_div').addClass('blink_me');
		//window.scroll(0,findPos(document.getElementsByClassName("product-view")));
		$('#loc_div').hide();
		Book_Service_Online_Fun_bangalore();
	}
}
...
function Book_Service_Online_Fun_bangalore()
{
	var return_validate=validate_refer_code();
	
	if($('#refcode').val()!="")
	{
		//alert(return_validate);
		var return_validate_arr=return_validate.split(" ");
		var referral_amount=return_validate_arr[3];
		//alert(referral_amount);
		//alert(battery_type);
		if(referral_amount.indexOf("available")>=0)
		{
			$('#refcode').focus();
			return;
		}				
		else if(referral_amount<300)
		{
			$('#refcode').focus();
			return;
		}				
		...
		{
			if(referral_amount<600 && battery_type=="Inverter Batteries")
			{
				//alert("inside");
				$('#refcode').focus();
				return;
...
function Book_Service_Online_Fun_step_2()
{
...
function Book_Service_Online_Fun_step_3()
{
...
function Book_Service_Online_Fun_step_4()
{
	...
	else if(order_product_payment_mode_val=="Cash On Delivery" || order_product_payment_mode_val=="Debit Card" || order_product_payment_mode_val=="Credit Card" || order_product_payment_mode_val=="Net Banking" || order_product_payment_mode_val=="UPI - Unified Payment Interface" || order_product_payment_mode_val=="Digital Wallets" || order_product_payment_mode_val=="Online Payment After Fitment")
	{
...
function Book_Service_Online_Fun_step_5()
{
	...
	var order_vehiclemake_URL = order_vehiclemake.replace(/ /g, "+");
	var order_vehiclemodel_URL = order_vehiclemodel.replace(/ /g, "+");	
	var order_product_type_URL = product_type.replace(/ /g, "+");
	var order_product_capacity_URL = product_capacity.replace(/ /g, "+");
	...
	order_product_capacity_URL=escape(order_product_capacity_URL);
	//alert(order_product_type_URL);
	//alert(order_product_capacity_URL);
	...
	$.ajax
	({					 
		url: "/bookbattery/servlet/Functions_Service?hidWhatToDo=Book_A_Service_Online",
		type: 'GET',
		data:{	UMN:order_usermobile_number_URL,
				UOTP:order_usermobile_otp,
				...
				ORD:order_number,
				USeT:order_services_type_URL,
				USPT:service_place_select_URL,
				UVMK:order_vehiclemake_URL,
				UVMD:order_vehiclemodel_URL,
				UPT:order_product_type_URL,
				UIC:order_product_capacity_URL,
				OPPM:order_product_payment_mode_URL,
				OQTY:order_product_qty_URL,
				ORC:refcode_URL,
...
function updateResult(data)
{
	 ...
	 //alert("product"+product_type);
	 
	 if(refcode!="")
	 {
		var return_validate_arr=data.split(" ");
		var referral_amount=return_validate_arr[3];
		...
		{
			//alert("success");
			$("#ref_code").html("<label id='ref_code_label' for='name' class='control-label' style='font-size: larger;color:#ff0000;'>Invalid Code.</label><table><tbody><tr></tr><tr id='inp_ref_code'><td><input type='text' value='"+refcode+"' name='name' class='form-control' onkeyup='javascript:backspace_press(event.key)' id='refcode' placeholder='Referral Code' style='border-bottom-color: #ff0000;color: #ff0000;font-size: 20px;'></td><td><span class='label label-danger' style='font-size: 14px;background-color: #ff0000;cursor: pointer;' id='applycode' onclick='javascript:validate_code()'>Not Eligible</span></td></tr></tbody></table>");
			return false;
		}				
		else if(referral_amount<300)
		{
			//alert("success");
			$("#ref_code").html("<label id='ref_code_label' for='name' class='control-label' style='font-size: larger;color:#ff0000;'>You have "+referral_amount+" Rs in your referral wallet. You cannot avail free battery health check.</label><table><tbody><tr></tr><tr id='inp_ref_code'><td><input type='text' value='"+refcode+"' name='name' class='form-control' onkeyup='javascript:backspace_press(event.key)' id='refcode' placeholder='Referral Code' style='border-bottom-color: #ff0000;color: #ff0000;font-size: 20px;'></td><td><span class='label label-danger' style='font-size: 14px;background-color: #ff0000;cursor: pointer;' id='applycode' onclick='javascript:validate_code()'>Not Eligible</span></td></tr></tbody></table>");
			return false;
		}				
		else
		{
			if(referral_amount<600 && product_type=="Inverter Batteries")
			{
				$("#ref_code").html("<label id='ref_code_label' for='name' class='control-label' style='font-size: larger;color:#ff0000;'>You have "+referral_amount+" Rs in your referral wallet. You cannot avail free Inerter battery health check.</label><table><tbody><tr></tr><tr id='inp_ref_code'><td><input type='text' value='"+refcode+"' name='name' class='form-control' onkeyup='javascript:backspace_press(event.key)' id='refcode' placeholder='Referral Code' style='border-bottom-color: #ff0000;color: #ff0000;font-size: 20px;'></td><td><span class='label label-danger' style='font-size: 14px;background-color: #ff0000;cursor: pointer;' id='applycode' onclick='javascript:validate_code()'>Not Eligible</span></td></tr></tbody></table>");
				return false;
			}
			else
			{
...
$(document).ready(function () {
	...
	isValid = true;
		...
		if (Book_Service_Online_Fun_step_2() == false)
		{
			return false;
		}
		else
	...
	if (Book_Service_Online_Fun_step_3() == false)
	{
		return false;
	}
	else
	...
	allNextBtn3.click(function(){
		...
		if (Book_Service_Online_Fun_step_4() == false)
		{
			return false;
		}
		else
    ...
    allNextBtn4.click(function(){
		...
		if (Book_Service_Online_Fun_step_5() == false)
		{
			return false;
		}
		else
...
function UpdateProduct_Filter()
{
	...
	//alert(publicUrl);
	var baseurldirectory =$("#baseurldirectory").val();
	//alert(baseurldirectory);
	var product_page_type =$("#product_page_type").val();
	//alert("product_page_type"+product_page_type);
	var product_state =$("#product_state").val();
	//alert(product_state);
	var product_city =$("#product_city").val();
	//alert(product_city);
	...
	if(product_page_type=="vehicle_model_list_page")
	{
		window.location.href = publicUrl+baseurldirectory+'batteryservice/'+product_state+'/'+product_city+'/'+$('#product_vehicle_make').val().replace(/ /g, '-')+'/';
	}
	else if(product_page_type=="service_list_page")
	{ 
		
		window.location.href = publicUrl+baseurldirectory+'batteryservice/'+product_state+'/'+product_city+'/'+$('#product_vehicle_make').val().replace(/ /g, '-')+'/'+$('#product_vehicle_model').val().replace(/ /g, '+').replace(/\//g, '_')+'/';
	}
	else if(product_page_type=="single_service_page")
	{ 
		window.location.href = publicUrl+baseurldirectory+'batteryservice/'+product_state+'/'+product_city+'/'+$('#product_vehicle_make').val().replace(/ /g, '-')+'/'+$('#product_vehicle_model').val().replace(/ /g, '+').replace(/\//g, '_')+'/'+$('#bat_type').val().replace(/ /g, '+')+'/'+$('#product_capacity').val().replace(/ /g, '+').replace(/\//g, '_')+'/'+$('#product_services_type_page').val().replace(/ /g, '+')+'/store/'+$('#Quantity').val().replace(/ /g, '+')+'/';
	}
	else{
		location.reload();
	}
...
function find_retaileres_avaliable()
{
	...
	$.ajax
	({					 
		//url: "/services/servlet/Functions?hidWhatToDo=Find_Retailer",
		url: "/bookbattery/servlet/Functions_Service?hidWhatToDo=Find_Retailer",
		type: 'GET',
		data:{
				USeT:order_services_type_URL,
				USPT:"Electrical Health Checkup",
				UAR:order_user_area_URL,
				UPN:order_user_pincode_URL,
				UST:order_state_URL,
				UCT:order_city_URL,
...
$(document).ready(function() {
	setTimeout(function(){
	$("[type='button'][onclick='javascript:Book_Service_Online_Fun()']").removeClass("btn").prop('disabled', false).html(" <span> Book Now</span>");
	}, 2000);
});
//########## Back Up code to enable Buy now button
